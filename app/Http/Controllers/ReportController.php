                ...
                $datatable->addColumn($col, function ($row) use ($tax, $type, $col, $group_taxes) {
                    ...
                    if ($type == 'sell') {
                        foreach ($row->sell_lines as $sell_line) {
                            if ($sell_line->tax_id == $tax['id']) {
                                if ($row->id == 35) {
                                    error_log('tax_id: ' . $sell_line->item_tax);
                                    error_log('tax_amount: ' . $tax_amount);
                                }
                                $tax_amount += ($sell_line->item_tax * ($sell_line->quantity - $sell_line->quantity_returned));
                                if ($row->id == 35)
                                    error_log($tax_amount);
                            }
                            if ($sell_line->tax_id_2 == $tax['id']) {
                                if ($row->id == 35) {
                                    error_log('tax_id_2: ' . $sell_line->item_tax);
                                    error_log('tax_amount: ' . $tax_amount);
                                }
                                $tax_amount += ($sell_line->item_tax_2 * ($sell_line->quantity - $sell_line->quantity_returned));
